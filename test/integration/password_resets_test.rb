...
class PasswordResetForm < PasswordResets
  def setup
    super #親クラスのdef setupを呼ぶ
    @user = users(:michael) #最初に情報を持っておく
    post password_resets_path,
        params:{password_reset: {email:@user.email}}
    @reset_user = assigns(:user) #createを実行した時点の@user
    #reset_digest , reset_sent_at, reset_tokenが入ってる
  end
end
...
class PasswordFormTest < PasswordResetForm
  ...
  test "reset with valid email" do
    ...
    #A = X.copy() , X = Y , B = X if not A == B みたいな感じ
  ...
  test "reset with wrong email" do
    ...
    #無効なリンク(email空)踏んだら，Homeにアクセスされる
    assert_redirected_to root_url
  end
  ...
  test "reset with right email and right token" do
    ...
    #DBじゃないのにself.reset_tokenの書き方ができるのは
    #modelでattr_accessor :reset_tokenをしているから
